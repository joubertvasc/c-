	...
	public sealed class RegistryKey : MarshalByRefObject, IDisposable
	{
		...
		public void DeleteSubKey(string subkey, bool throwOnMissingSubKey)
		{
			...
			else
			{
				if(CheckHKey())
				{
					if(RegDeleteKey(m_handle, subkey)!=0)
					{
            ...
            if (m_writable && value != null)
			{
				if(CheckHKey())
				{
					...
					switch(value.GetType().ToString())
					{
						case "System.String":
							type = KeyType.REG_SZ;
							data = System.Text.Encoding.Unicode.GetBytes((string)value);
							break;
						case "System.Byte[]":
							type = KeyType.REG_BINARY;
							data = (byte[])value;
							break;
						case "System.Int32":
							type = KeyType.REG_DWORD;
