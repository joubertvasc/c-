...
namespace JVUtils
{
    public static class GoogleCellID
    {
        ...
        // Overloaded methods
        public static bool GetLatLng(int MCC, int MNC, int LAC, int CID, ref double latitude, ref double longitude)
        {
            string[] args = new string[4];
            args[0] = Convert.ToString(MCC);
            args[1] = Convert.ToString(MNC);
            args[2] = Convert.ToString(LAC);
            args[3] = Convert.ToString(CID);

            return GetLatLng(args, ref latitude, ref longitude);
        }

        public static bool GetLatLng(double MCC, double MNC, double LAC, double CID, ref double latitude, ref double longitude)
        {
            string[] args = new string[4];
            args[0] = Convert.ToString(MCC);
            args[1] = Convert.ToString(MNC);
        ...
        public static bool GetLatLng(decimal MCC, decimal MNC, decimal LAC, decimal CID, ref double latitude, ref double longitude)
        {
            string[] args = new string[4];
            args[0] = Convert.ToString(MCC);
            args[1] = Convert.ToString(MNC);
        ...
        public static bool GetLatLng(string MCC, string MNC, string LAC, string CID, ref double latitude, ref double longitude)
        {
            string[] args = new string[4];
            args[0] = MCC;
            args[1] = MNC;
        ...
        // Main method
        static public bool GetLatLng(string[] args, ref double latitude, ref double longitude)
        {
            ...
            try
            {
                ...
                if (res.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    if (ret_code == 0)
                    {
                        latitude = ((double)((ps[7] << 24) | (ps[8] << 16) | (ps[9] << 8) | (ps[10]))) / 1000000;
                        longitude = ((double)((ps[11] << 24) | (ps[12] << 16) | (ps[13] << 8) | (ps[14]))) / 1000000;
