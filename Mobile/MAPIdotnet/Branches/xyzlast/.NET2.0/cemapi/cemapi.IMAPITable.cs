...
namespace MAPIdotnet
{
    internal static partial class cemapi
    {
        ...
        private struct SRowSet
        {
            ...
            public byte[] AsBytes
            {
                get
                {
                    ...
                    Marshal.Copy(aRow, b, 0, (int)this.cRows);
                    return b;
                }
            }
        }
        ...
        private class MAPITable : MAPIUnknown, IMAPITable
        {
            ...
            [DllImport("MAPIlib.dll", EntryPoint = "IMAPITableQueryPosition")]
            private static extern HRESULT pIMAPITableQueryPosition(IntPtr table, ref uint lpulRow);
            ...
            public SRow[] QueryRows(int lRowCount)
            {
                ...
                if (hr == HRESULT.MAPI_E_CALL_FAILED)
                {
                ...
                IntPtr rows = (IntPtr)(((uint)pRowSet) + 4);

                for (int i = 0; i < cRows; i++)
                {
                    uint pRowOffset = (uint)(rows) + (uint)(i * SizeOfSRow);
                    uint cValues = (uint)Marshal.ReadInt32((IntPtr)(pRowOffset + 4));
                    uint pProps = (uint)Marshal.ReadInt32((IntPtr)(pRowOffset + 4 * 2));
            ...
            public uint QueryPosition()
            {
                uint pos = 0;
                HRESULT hr = pIMAPITableQueryPosition(this.ptr, ref pos);
                if (hr != HRESULT.S_OK)
                    throw new Exception("pIMAPITableQueryPosition failed: " + hr.ToString());
                return pos;
            ...
            public void SortRows(PropTags tag, TableSortOrder order)
            {
                ...
                IntPtr sortArray = Marshal.AllocHGlobal(4 * 3 + sizeS);
                Marshal.WriteInt32(sortArray, 1);
                Marshal.WriteInt64(sortArray, 4, 0);
                SSortOrder s;
                s.ulOrder = order;
                s.ulPropTag = tag;
                Marshal.StructureToPtr(s, (IntPtr)(((uint)sortArray) + 4 * 3), false);
        ...
        private static SBinary SBinaryCreate(byte[] data)
        {
            ...
            int len = data.Length;
            b.cb = (uint)len;
            b.lpb = Marshal.AllocHGlobal(len);
            Marshal.Copy(data, 0, b.lpb, len);
            return b;
        }
