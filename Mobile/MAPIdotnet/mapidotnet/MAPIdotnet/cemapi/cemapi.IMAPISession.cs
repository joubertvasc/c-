...
namespace MAPIdotnet
{
    internal static partial class cemapi
    {
        public interface IMAPISession : IMAPIUnknown, IDisposable
        {
            ...
            void DisplayMessage(byte[] msgId);
        }
        ...
        private class MAPISession : MAPIUnknown, IMAPISession
        {
            ...
            public void DisplayMessage(byte[] msgId)
            {
                IntPtr lpEntryID = Marshal.AllocHGlobal(msgId.Length);
                Marshal.Copy(msgId, 0, lpEntryID, msgId.Length);
                HRESULT hr = (HRESULT)MailDisplayMessage(lpEntryID, (uint)msgId.Length);
                Marshal.FreeHGlobal(lpEntryID);
                if (hr != HRESULT.S_OK)
                    throw new Exception("MailDisplayMessage failed: " + hr.ToString());
            }
        }
    }
}
