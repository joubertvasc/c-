...
#warning MAPIFolder dispose not done!
        ...
        private cemapi.IMAPITable Hierarchy
        {
            get
            {
                ...
                return this.folder.GetHierarchyTable();
            }
        }
            ...
            get
            {
                if (this.contents == null)
                {
                    ...
                    contents.SetColumns(new cemapi.PropTags[] { cemapi.PropTags.PR_ENTRYID,
                        cemapi.PropTags.PR_MESSAGE_DELIVERY_TIME});
                }
        ...
        public int MessagesCursor()
        {
            return (int)this.Contents.QueryPosition();
        }
        ...
        {
            int num = msgs.Length;
            byte[][] msgIds = new byte[num][];
            for (int i = 0; i < num; ++i)
                msgIds[i] = msgs[i].AsByteArray;
        ...
        public IMAPIFolder CreateFolder(string name, bool openIfExist)
        {
            cemapi.IMAPIFolder folder = this.folder.CreateFolder(name, openIfExist);
            IMAPIFolderID id = new MAPIFolderID(folder.GetProps(new cemapi.PropTags[] { cemapi.PropTags.PR_ENTRYID })[0].AsBinary, this.parentStore);
            return new MAPIFolder(this.parentStore, folder, id);
        }

        public DeleteFolderResult DeleteFolder(IMAPIFolderID folderId, bool delSubFolders, bool delMsgs)
        {
            return this.folder.DeleteFolder(folderId.AsByteArray, delSubFolders, delMsgs);
        }

        public void CopyMessages(IMAPIMessageID[] messages, IMAPIFolder destFolder, bool moveMsgs)
        {
            byte[][] msgIds = new byte[messages.Length][];
            for (int i = 0, len = messages.Length; i < len; i++)
                msgIds[i] = messages[i].AsByteArray;
            this.folder.CopyMessages(msgIds, ((MAPIFolder)destFolder).folder, moveMsgs);
        }
