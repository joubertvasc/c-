...
HRESULT IMAPITableQueryPosition(IMAPITable *table, ULONG *lpulRow)
{
	return table->QueryPosition(lpulRow, NULL, NULL);
}
...
HRESULT IMAPIPropOpenProperty(IMAPIProp *prop, ULONG ulPropTag, ULONG flags, IUnknown **lppUnk)
{
	return prop->OpenProperty(ulPropTag, NULL, 0, flags, lppUnk);
}
...
HRESULT IMAPIFolderCreateFolder(IMAPIFolder *folder, LPTSTR lpszFolderName, ULONG flags, IMAPIFolder **newFolder)
{
	return folder->CreateFolder(0, lpszFolderName, NULL, NULL, flags, newFolder);
}

HRESULT IMAPIFolderDeleteFolder(IMAPIFolder *folder, ULONG cbEntryID, LPENTRYID lpEntryID, ULONG flags)
{
	return folder->DeleteFolder(cbEntryID, lpEntryID, 0, NULL, flags);
}

HRESULT IMAPIFolderCopyMessages(IMAPIFolder *folder, LPENTRYLIST lpMsgList, LPVOID lpDestFolder, ULONG flags)
{
	return folder->CopyMessages(lpMsgList, NULL, lpDestFolder, 0, NULL, flags);
}
...
HRESULT IMessageOpenAttach(IMessage *msg, ULONG num, IAttach **attachment)
{
	return msg->OpenAttach(num, NULL, 0, attachment);
}