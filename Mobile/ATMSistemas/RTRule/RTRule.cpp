...
//
// ************************************************************
// RTRule.CPP -> Original MapiRule.CPP
// 
...
// MS Original -> // {3AB4C10E-673C-494c-98A2-CC2E91A48115}
// MS Original -> DEFINE_GUID(CLSID_RTRule, 0x3ab4c10e, 0x673c, 0x494c, 0x98, 0xa2, 0xcc, 0x2e, 0x91, 0xa4, 0x81, 0x15);

// {687A5082-B5CD-47c4-975D-A9CE20BE3726}
DEFINE_GUID(CLSID_RTRule, 0x687a5082, 0xb5cd, 0x47c4, 0x97, 0x5d, 0xa9, 0xce, 0x20, 0xbe, 0x37, 0x26);

/* Add to apps.reg:
[HKEY_CLASSES_ROOT\CLSID\{687A5082-B5CD-47c4-975D-A9CE20BE3726}\InProcServer32]
@="RTRule.dll"
*/
...
"{687A5082-B5CD-47c4-975D-A9CE20BE3726}"=dword:1
*/
...
TCHAR* getPath()
{
	...
	if (RegOpenKeyEx(HKEY_LOCAL_MACHINE,TEXT("\\Software\\Microsoft\\Inbox\\Rules\\Configuration"),0, KEY_READ, &key)==ERROR_SUCCESS) 
	{
		RegQueryValueEx(key, L"Command", NULL, &dwType, (PUCHAR)szBuffer, &dwLen);
	}
            ...
            ULONG cbDestFolder, LPENTRYID lpDestFolder, ULONG *pulEventType, MRCHANDLED *pHandled)
{
	...
	// If the size of the sender is smaller then 6 characters, send message to ATM
    int s = 0;
    for (int i=0; i < sizeof(pspvEmail->Value.lpszW); i++)
	    s++;

	// Here we filter the message on some predetermined string. For sample purposes
	// here we use "rt#", original MS ->"zzz". What happens when the filter condition(s) are met is up to
	...
	if (wcsstr(pspvSubject->Value.lpszW, L"rt2#") != NULL ||
		wcsstr(pspvSubject->Value.lpszW, L"RT2#") != NULL ||
		wcsstr(pspvSubject->Value.lpszW, L"Rt2#") != NULL ||
		wcsstr(pspvSubject->Value.lpszW, L"rT2#") != NULL ||
		s < 6)
	{
		...
		// Prepare the message to be passed as parameter to rt.exe
		TCHAR szMessage[255];
		TCHAR szMessageToSend[255];
        memset(szMessage,0,sizeof(szMessage));
        memset(szMessageToSend,0,sizeof(szMessageToSend));

		if (s < 6)
			_tcscat(szMessage, TEXT("rt#'retornotim,"));

		_tcscat(szMessage, pspvSubject->Value.lpszW);

		if (s < 6)
			_tcscat(szMessage, TEXT("'"));
		
		_tcscat(szMessage, TEXT("@@"));
        _tcscat(szMessage, pspvEmail->Value.lpszW);

        for (int i=0; i < sizeof(szMessage); i++)
		{
            if ((szMessage[i] == putchar(13)) || (szMessage[i] == putchar(10)))
                szMessage[i] = ' ';

			if (szMessage[i] == putchar(34))
				szMessage[i] = putchar(39);
		}

		_tcscat(szMessageToSend, TEXT("/message:\""));
	    _tcscat(szMessageToSend, szMessage);
	    _tcscat(szMessageToSend, TEXT("\""));

		// Get the RT.exe path from registry
		TCHAR* rt = getPath();
        //MessageBox(NULL, szMessageToSend, TEXT("Message"), MB_OK);

       if (FALSE == CreateProcess(rt, szMessageToSend, NULL, NULL, NULL, 0, NULL, NULL, NULL, &pi))
       {
//            goto Error;
            MessageBox(NULL, TEXT("Could not start rt.exe"), TEXT("Error"), MB_OK);		
       }
...
STDAPI DllRegisterServer()
{
    ...
    lr = RegCreateKeyEx(HKEY_CLASSES_ROOT, TEXT("\\CLSID\\{687A5082-B5CD-47c4-975D-A9CE20BE3726}"),
	                              0, NULL, 0, 0, NULL, 
	                              &hKey, &dwDisposition);
    if (lr != ERROR_SUCCESS)
    {
    ...
    lstrcpy(wszValue, TEXT("rtrule.dll"));
    ...
    dwDisposition = 1;
    lr = RegSetValueEx(hKey, TEXT("{687A5082-B5CD-47c4-975D-A9CE20BE3726}"), 0, REG_DWORD, 
...
STDAPI DllUnregisterServer()
{
    ...
    RegDeleteKey(HKEY_CLASSES_ROOT, TEXT("\\CLSID\\{687A5082-B5CD-47c4-975D-A9CE20BE3726}"));
    ...
    RegDeleteValue(hKey, TEXT("{687A5082-B5CD-47c4-975D-A9CE20BE3726}"));
