...
namespace WebSecurity
{
    ...
    internal class Capture : ISampleGrabberCB, IDisposable
    {
        ...
        public Capture(int iDeviceNum, int iWidth, int iHeight, short iBPP, Control hControl)
        {
            ...
            {
                throw new Exception("There are video capture devices found!");
            }
            else if (iDeviceNum + 1 > capDevices.Length)
            {
        ...
        private void CloseInterfaces()
        {
            ...
            {
                Debug.AddLog("CloseInterfaces, " + ex.ToString(), true);
            }
        ...
        int ISampleGrabberCB.BufferCB( double SampleTime, IntPtr pBuffer, int BufferLen )
        {
            ...
            System.Diagnostics.Debug.Assert(BufferLen == Math.Abs(m_stride) * m_videoHeight, "Incorrect buffer length");

            if (m_WantOne)
            {
                m_WantOne = false;
                System.Diagnostics.Debug.Assert(m_ipBuffer != IntPtr.Zero, "Unitialized buffer");
